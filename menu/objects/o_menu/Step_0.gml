...
if(keyboard_check_pressed(confirmButt))
{
	...
	{
		oTransition.target = LearningLevels; SlideTransition(TRANS_MODE.GOTO); 
	}
	
	if(selected == 1) // How to Play Room
	{
		oTransition.target = HowToPlay; SlideTransition(TRANS_MODE.GOTO);
	}
	
	if(selected == 2) // Options Room
	{
		oTransition.target = Options; SlideTransition(TRANS_MODE.GOTO);
	}
