...
//Draw elements only if it's permitted to
if(draw) {
	// Set default font
	...
	for(i = 0; i < array_length_1d(global.options); i++)
	{
		...
		// Draw the text
		draw_text_transformed((width/2)-220, (height/2 - 100) + (fa_center+i*spacing)-100, global.options[i], 4, 4, 0);
	...
	draw_sprite(s_cursor, -1, ((width/2)-240) + cursorLevitate - cursWidth/2,(((height/2)-200) + selectLerp*spacing)-65);
}